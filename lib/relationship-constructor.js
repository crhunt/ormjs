...
function update_rolebox_group_relationships(rel) {
    var dr = rel.datum();
    var from_obj = get_any_object( dr.from );
    var to_obj = get_any_object( dr.to );
    var rbgroup;
...
function svg_mouseup(event, rel) {
    ...
    if ( to_what.found ) {
        if ( to_what.distance < to_what.tolerance &&
            ...
            // Get overlay to link to
            to_pos = closest_overlay(get_any_object(dr.from).datum(), 
    ...
    if ( dr.to != "" ) {
        // Keep relation
        // 1. Set what type of relation to draw based on what we're connecting
        set_default_reltype(rel);
        // 2. Draw the relationship
        draw_relationship(rel);
        // 3. Make a record of the relationship
