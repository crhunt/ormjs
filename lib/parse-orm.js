...
class Reading {
    ...
    constructor(n,rbgID) {
        this.id = `${rbgID}-reading-${n}`;
        this._rbgID = rbgID;
    }

    set_reading(readingtype) {
...
class ReadingOrder {
    ...
    constructor(n,rbgID) {
        this.id = `${rbgID}-reading-order-${n}`;
        this._rbgID = rbgID;
    }

    set_sequence(direction) {
        if(direction == "forward") {
            var obj = metamodel_forward_reading_order( orm.rbgroups[this._rbgID] );
            for (var key in obj) { this[key] = obj[key]; }
        }
    }
...
class Fact {
    ...
    constructor(n, rbgID) {
        ...
        this._rbgID = rbgID;
        this.FactRoles = metamodel_add_roles(this.id, orm.rbgroups[rbgID]);
        this.InternalConstraints = [];
        this.ReadingOrder = metamodel_reading_orders( orm.rbgroups[rbgID] );
        this._Name = orm.rbgroups[rbgID].datum().name;
        this._Shadows = [ rbgID ];
        this.populate_shadows();
    }

    populate_shadows() {
        // Get shadowing information
        var matchlist = metamodel_match_fact( orm.rbgroups[this._rbgID], this.id );
...
function metamodel_match_fact(rbgroup, excludeID) {
    ...
    for (var factID in metamodel.Fact) {
        ...
        if (match && factID != excludeID) { factlist.push( factID ); }
    }
...
function metamodel_fact(mmID, rbgroup) {
    ...
    var obj = {
        ...
        ReadingOrder: metamodel_reading_orders(rbgroup),
        InternalConstraints: [],
...
function metamodel_reading_orders(rbgroup) {
    ...
    var ronum = 0;
    var ro = new ReadingOrder(ronum,rbgroup.attr("id"));
    //console.log("ro", ro)
...
function metamodel_forward_reading_order(rbgroup) {
    ...
    var r = new Reading(0,rbgroup.attr("id"));
    //console.log("r 1", r);
