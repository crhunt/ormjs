...
function initialize_metamodel() {
    metamodel = { Object: {}, Fact: {}, SubtypeFact: {}, Constraint: {}, relIDs: [] };
    ormjs2mm = {};
}
...
function metamodel_forward_reading(reading) {
    ...
    // Only connections to entit/ies that are not mandatory
    var connIDs = rs.map( (e) => {
        var connID;
        factroles[e].IsMandatory ? connID = null 
                                 : connID = find_EtoRB_connector(factroles[e]._rboxID);
        return connID          
    } ).filter( v => v);
    includedIDs.push.apply(includedIDs,connIDs);
    console.log("fwd includedIDs", includedIDs)
...
function metamodel_mandatory_reading(reading, rolen) {
    ...
    // Only connections to entit/ies that are not mandatory
    includedIDs.push( find_EtoRB_connector(metamodel.Fact[reading._factID].FactRoles[rolen]._rboxID) );
    console.log("mand includedIDs", includedIDs)

    //var includedIDs = rs.map( e => "c-"+factroles[e]._rboxID );
    //includedIDs.push.apply(includedIDs, [...fwd_reading._includedIDs]);
