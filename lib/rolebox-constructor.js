...
function duplicate_fact(rbgroup) {
    ...
    // Duplicate fact
    var rbcopy = draw_fact(x,y);
    var gdc = rbcopy.datum()

    // Duplicate roleboxes
    var boxes = [...gd.boxes];
    if (gd.flipped) { boxes.reverse(); }
    duplicate_rolebox(boxes[0], gdc.boxes[0])
    var rng = range(boxes.length-1, startAt=1);
    for ( var n in rng ) {
        add_rolebox(rbcopy);
        duplicate_rolebox(boxes[rng[n]], gdc.boxes[rng[n]]);
    }
    ...
    set_rolebox_display_name(rbcopy);
...
function duplicate_rolebox(rboxID, crboxID) {
    var crbox = orm.roleboxes[crboxID];
    var rbox = orm.roleboxes[rboxID];
    // Match datum 
    crbox.datum().name = rbox.datum().name;
    crbox.datum().multiplicity = rbox.datum().multiplicity;
    // Set box visualization to match data
    set_rb_uniqueness_constraint(crbox);
}
...
function find_EtoRB_connector(rboxID) {
    /* Find the connector that connects from from_id to to_id */

    var connlist = orm.roleboxes[rboxID].datum().connectors;
...
function delete_rolebox(rbox) {
    ...
    // Remove connectors
    var conns = [...rbox.datum().connectors];
    conns.map( (connID) => {
        // Remove connector reference from parent
        gd.connectors = remove_from_array( gd.connectors, connID );
    ...
    // Remove the rolebox group from records
...
function delete_fact(rbgroup) {
    ...
    conns.map( (connID) => { orm.connectors[connID].delete(); });
...
function rbdragged(event,d,rbgroupID) {
    ...
    if (d3.select("#"+rbgroupID).classed("selected")) {
        drag_selected(event);
    } else {
...
function propagate_connection_data(object) {

    if( object.datum().kind == "rolebox" ) {
        // Set entity for object
        set_entity(object);
        // Set connectors at the group level
        var rbgroup = d3.select("#"+object.attr("parent"));
        bubble_connectors(rbgroup);
        // Set primary entity based on connectors
...
function rbomousedown(event) {
    ...
    /* Note: this prevents a bug that can sometimes leave a connector behind when someone has
       clicky fingers. */
    ...
    // mouse position
    var pos = { x1: rbo.x, x2: mousepos.x, y1: rbo.y, y2: mousepos.y };
...
function eligible_rolebox_locations(boxID, linkto="entity", loc=dragevent.locations) {
...
function rolebox_position(rbox) {
    ...
    var position = { relative: null, absolute: null, number: boxes.length };
