...
function add_rolebox(rbgroup) {
    ...
    redraw_relationships(d.relationships);
...
function align_roleboxes(rbgroup) {
    ...
    for (var n in gd.boxes) {
        ...
        // Set eligible overlays for relationships
...
function rotate_rolebox_group(rbgroup) {
    ...
    if ( d.rotated ) {
        rbgroup
            .attr("transform", () => translate_rotate(d.x,d.y,0,0));
    } else {
        rbgroup
...
function rbomousedown(event) {
    ...
    if (gd.rotated) {
        mousepos = {x: m[0] + d.x - rb_param.height/2, 
                    y: m[1] + d.y - rb_param.width/2};
    }

    // Center of closest entity overlay
    var rbo = closest_rboverlay(mousepos, boxID);
    console.log(mousepos, rbo)
