...
class Constraint {
    ...
    redraw() {
        ...
        // Set radius data
        var size = this.calculate_radius();
        // Group
        d3.select(`#${this.id}`)
            .attr("width", 2*size.radius)
            .attr("height", 2*size.radius);
        // Overlay
        d3.select(`#o-${this.id}`)
            .attr("r", size.oradius);
    ...
    calculate_radius() {
        ...
        // This will be modified when we support value constraints
        ...
        if (this.d3object.datum().content.length > 0) {
            var dr = or-r;
            var nr = (this.d3object.datum().content.length*10 + 6)/2;
            r = Math.max(nr, r);
            or = r+dr;
        }

        return {radius: r, oradius: or}
    }

    static supported_types() {
        return ["inclusive-or", "exclusion", "exclusive-or", "equality",
                "identifier", "preferred-identifier", "subset", "external-constraint"]
    }
    ...
    can_connect(d3target) {
        ...
        if (d.kind == "rolebox") {
            ...
            if (ctd.type == constrainttypes.identifier || 
                ...
                ctd.type == constrainttypes.external_frequency ) {
