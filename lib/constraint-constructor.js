...
tolerance.link["constraint"] = GraphUtils.get_css_number('--constraint-oradius');
tolerance.snap["constraint"] = GraphUtils.get_css_number('--constraint-radius')/5;
...
class Constraint {
    ...
    ref = "constraints";
    param = {
        oradius : GraphUtils.get_css_number('--constraint-oradius'),
        radius : GraphUtils.get_css_number('--constraint-radius'),
        stroke : GraphUtils.get_css_number('--stroke-width')
    };
    ...
    create_d3object(x,y) {
        ...
        var constrainttypes = Constraint.supported_types().names;
        ...
        // Move to position
        constraint.attr("transform", () => Graph.translate(x,y));
    ...
    set_radius() {
        ...
        // Default radius
        var r = GraphUtils.get_css_number('--constraint-radius');
        var or = GraphUtils.get_css_number('--constraint-oradius');
    ...
    dragstarted(event,d) {
        var constraint = orm.constraints[d.id];
        Graph.dragstarted(event,d,constraint);
    }

    dragged(event, d) {
        var constraint = orm.constraints[d.id];
        Graph.dragged(event,d,constraint);
    }

    dragended(event,d) {
        var constraint = orm.constraints[d.id];
        Graph.dragended(constraint);
    }
    ...
    mousedown(event, constraint) {
        ...
        Graph.mousedown(event, constraint, mousepos);
    }
    ...
    static remove(event) {
        ...
        if (event.ctrlKey || event.buttons == 2) {
            ...
            var parentID = Graph.get_parentID(click_objID);
