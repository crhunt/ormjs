...
ormjs.View = class {
        ...
        this.d3object
            ...
            .on("dblclick", (event) => {
                ...
                if (event.shiftKey && event.ctrlKey) {
                    ...
                    new ormjs.Value({x: mouse[0], y: mouse[1]})
                } else if (event.shiftKey) {
                    // Create rolebox on shift+doubleclick
                    new ormjs.Predicate({x: mouse[0], y: mouse[1]})
                } else {
                    // Create entity on doubleclick
                    new ormjs.Entity({x: mouse[0], y: mouse[1]})
                }
            });
    ...
    objects_in_view() {
        ...
        var objIDs = object_classes.map( (cls) => {
            var object_divs = d3.select(this.id).selectAll("."+cls);
            for ( var k in object_divs.nodes() ) { 
                // Add object id
                return object_divs.nodes()[k].id;
            }
        });
        return objIDs
    }
