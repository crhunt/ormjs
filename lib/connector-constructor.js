...
class Connector {
    ...
    can_connect() {
        ...
        if ( d_from.kind == "constraint" ) {
            if ( d_to.kind != "rolebox" ) { return false }
            // TO DO: Check allowed number of connections
    ...
    set_eligible_overlays() {
        ...
        */
        ...
        if ( d_to.kind == "rolebox" ) {
            cd.to_loc = eligible_rolebox_locations(cd.to, d_from.kind);
        }
        if ( d_from.kind == "rolebox" ) {
            cd.from_loc = eligible_rolebox_locations(cd.from, d_to.kind);
        }
    }
    ...
    assign_role() {
        ...
        d_conn.to_loc = eligible_rolebox_locations(d_conn.to, d_object.kind);
    ...
    svg_mouseup(event) {
        ...
        console.log("to_what", to_what.object.attr("id"), cd.from);
