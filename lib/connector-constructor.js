...
class Connector {
    ...
    constructor(data, d3object=false) {
    ...
    can_connect() {
        ...
        if ( d_from.kind == "constraint" ) {
            ...
            var roles = orm.constraints[ cd.from ].roles();
            if (roles.includes(cd.to)) { return false }
            // Set connect constraint --> rolebox
            cd.conntype = conntypes.CtoRB;
            return true
        }
    ...
    update_location() {
        ...
        var from_pos = closest_overlay( get_any_object( cd.to ).datum(), cd.from, cd.from_loc);
        var to_pos = closest_overlay( get_any_object( cd.from ).datum(), cd.to, cd.to_loc );
        // Move the connector
        cd.x1 = from_pos.x;
        cd.y1 = from_pos.y;
        cd.x2 = to_pos.x;
    ...
    assign_role() {
        ...
        /* Use connector data to assign a role to an entity or value */
        ...
        if (d_object.kind == "constraint") {
            ...
            /*var loc = closest_overlay({x:d_conn.x2, y:d_conn.y2}, d_conn.to);
            var options = { "left": ["left"], "right": ["right"], 
                            "top": ["top", "bottom"], "bottom": ["top", "bottom"] };
            d_conn.to_loc = options[ loc.location ];*/
            d_conn.to_loc = dragevent.locations;
            this.d3object.lower(); // Put beneath everything
            console.log("to_loc", d_conn.to_loc);
        }
    ...
    data_propagate(detach=false) {
        ...
        */
        ...
        console.log("data_propagate", from_obj.attr("id"), to_obj.attr("id"))
        ...
        console.log("rolebox datum", to_obj.datum());
        console.log("obj datum", from_obj.datum());
