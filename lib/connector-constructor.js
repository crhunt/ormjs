...
class Connector {
    ...
    paths() {
        console.log("children", this.d3object._groups[0][0].childNodes)
        ...
        if ( cd.conntype == conntypes.default ) {
            ...
            if ( draw_arrows.includes(type) && cd.directed ) {
                ...
                connover.attr("d", function() { return constraintArrowPath(lineparam.length) })
                        .attr("class",`connector ${connclass.CtoRB} object${np}`);
                conn
    ...
    svg_mousemove(event) {
        ...
        */
        ...
        var connover = d3.select(`#o-${this.id}`);
        connover.attr("d", null);
        connover.attr("d", d3.line()([ [cd.x1, cd.y1], [cd.x2, cd.y2] ]));*/
    }
