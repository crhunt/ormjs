...
tolerance.link["connector"] = GraphUtils.get_css_number('--stroke-width-wide');
...
class Connector {
    ...
    static redraw(conns) {
        var conntypes = Connector.supported_types().names;
        conns.map( (connID) => {
    ...
    create_d3object(pos) {
        ...
        var conninfo = Connector.supported_types();
        ...
        connector.append("path")
            .attr("class", "connector "+conninfo.classes[conntype])
            .attr("id","p-"+this.id)
    ...
    can_connect() {
        ...
        if ( d_from.kind == "entity" ) {
            var entity = orm.entities[ cd.from ];
    ...
    update_location() {
        ...
        var to_pos = Graph.closest_overlay( d3.select("#"+cd.from).datum(), cd.to, cd.to_loc);
        var from_pos = Graph.closest_overlay( to_pos, cd.from, cd.from_loc);
    ...
    set_eligible_overlays() {
        ...
        */
        ...
        if ( d_to.kind == "rolebox" ) {
            ...
            cd.to_loc = orm.roleboxes[cd.to].eligible_locations(d_from.kind, cd.to_loc);
            
        }
        if ( d_from.kind == "rolebox" ) {
            //cd.from_loc = eligible_rolebox_locations(cd.from, d_to.kind, cd.from_loc);
            cd.from_loc = orm.roleboxes[cd.from].eligible_locations(d_to.kind, cd.from_loc);
        }
    }
    ...
    assign_role() {
        ...
        if (d_object.kind == "entity" || d_object.kind == "value") {
            ...
            orm.rbgroups[d_rolebox.parent].set_subject();
            // Check flip condition for rolebox group and redraw connected connectors
            orm.rbgroups[d_rolebox.parent].update_flip();
            // Set eligible overlay locations for linking connector
            d_conn.to_loc = orm.roleboxes[d_conn.to].eligible_locations(d_object.kind, d_conn.to_loc);
        ...
        if (d_object.kind == "constraint") {
            ...
            var closest_loc = Graph.closest_overlay({x:d_conn.x2, y:d_conn.y2}, d_conn.to);
            ...
            d_conn.to_loc = orm.roleboxes[d_conn.to].eligible_locations(d_object.kind, data);
    ...
    static all_subtypes() {
        ...
        for (var connID in orm.connectors) {
            if (orm.connectors[connID].d3object.datum().conntype == conntypes.subtype) {
                subtypes[connID] = orm.connectors[connID];
            }
        }
        return subtypes
    }
    ...
    actions() {
        ...
        connover
            ...
            .on("mouseover", (d) => { 
                Graph.class_as(`p-${id}`, "conn_mouseover"); 
                Graph.class_as(`o-${id}`, "conn_mouseover"); 
            })
            .on("mouseout", (d) => { 
                Graph.unclass_as(`p-${id}`, "conn_mouseover"); 
                Graph.unclass_as(`o-${id}`, "conn_mouseover"); 
            });
    }
    ...
    svg_mousemove(event) {
        ...
        */
        ...
        var pos = Graph.closest_overlay({x: cd.x2, y: cd.y2}, cd.from);
        cd.x1 = pos.x;
        cd.y1 = pos.y;
    ...
    svg_mouseup(event) {
        ...
        var to_what = Graph.closest_object(mouse_position);
    ...
    static remove(event) {
        ...
        if (event.ctrlKey || event.buttons == 2) {
            ...
            try{
                orm.connectors[ Graph.get_parentID(click_objID) ].delete();
            }
            catch (e) {
    ...
    constraint_connector_delete() {
        ...
        if ( d.conntype == conntypes.subtype ) {
            ...
            conns.map( (connID) => {
                orm.connectors[connID].simple_delete();
            });
        }
    }
    ...
    data_propagate(detach=false) {
        ...
        */
        ...
        if (from_obj != null) {
            ...
            detach ? fr = GraphUtils.remove_from_array(fr, this.id ) // Remove conn from list
                   : fr.push(this.id); // Add conn to list
        }
        if (to_obj != null) {
            var tr = to_obj.datum().connectors;
            // Remove conn from lists
            detach ? tr = GraphUtils.remove_from_array(tr, this.id ) // Remove conn from list
                   : tr.push(this.id); // Add conn to list
        }
        
        // Add/Remove from rolebox group datum
        if (from_obj.datum().kind == "rolebox") {
            orm.roleboxes[cd.from].bubble_connectors();
        }
        if (to_obj.datum().kind == "rolebox") {
            orm.roleboxes[cd.to].bubble_connectors();
        }
...
function closest_on_line(pos,linepos) {
    // Line equation
    ...
    var ipos = {x: ix, y:iy};
    // On line segment
    ipos = position_in_box(ipos,linepos);
    // Distance from position to intercept
    distance = Math.sqrt( Math.pow( (pos.x - ipos.x), 2) +
                          Math.pow( (pos.y - ipos.y), 2) );
    return {x: ipos.x, y:ipos.x, distance: distance}
}

function position_in_box(pos, boxpos) {
