...
ormjs.Connector = class {
    ...
    record() {
        ...
        ormjs.models[this.model].objects.connector[this.id] = this;
    }
    ...
    assign_role() {
        ...
        if (d_object.kind == "entity" || d_object.kind == "value") {
            ...
            objects["rolebox_group"][d_rolebox.parent].set_subject();
            // Check flip condition for rolebox group and redraw connected connectors
            objects["rolebox_group"][d_rolebox.parent].update_flip();
    ...
    svg_mousemove(event) {
        ...
        */
        ...
        var pos = ormjs.Graph.closest_overlay({x: cd.x2, y: cd.y2}, cd.from);
        cd.x1 = pos.x;
        cd.y1 = pos.y;
