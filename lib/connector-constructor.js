...
function update_rolebox_group_connectors(conn, remove=false) {
    ...
    if ( to_obj.datum().kind == "rolebox" ) {
        ...
        if ( from_obj.datum().kind == "value" && remove ) {
            // Rolebox no longer connects to a value
            to_obj.datum().entity = null;
        }
    }
}
...
function check_orm_logic(conn) {
    ...
    /* 
        ...
        This is a fussy bit of logic that checks the
    ...
    else if ( (d_to.kind == "entity" || d_to.kind == "value") && 
              d_from.kind == "rolebox" ) {
        /* Rolebox to Entity */
        
        // Only one entity is allowed per rolebox
        if ( d_from.entity == null ) {
            // We always define connections as from entity/value to rolebox
            ...
            // Assign the role
            d_from.kind == "entity" ? assign_role_to_entity(d_to, cd) 
                                    : assign_role_to_value(d_to, cd);
            if ( cd.to == "" ) { return }
        } else {
    ...
    } else if ( d_to.kind == "rolebox" && 
                (d_from.kind == "entity" || d_from.kind == "value") ) {
        /* Entity to Rolebox */

        // Only one entity is allowed per rolebox
        if ( d_to.entity == null ) {
            d_from.kind == "entity" ? assign_role_to_entity(d_to, cd) : assign_role_to_value(d_to, cd);
            if ( cd.to == "" ) { return }
        } else {
            // The rolebox is already connected to an entity.
            // This connection isn't allowed.
...
function find_connector(from_id, to_id) {
    /* Find the connector that connects from from_id to to_id */

    var connlist = orm.roleboxes[to_id].datum().connectors;
    for (var n in connlist) {
        if ( orm.connectors[connlist[n]].datum().from == from_id ) { return connlist[n] }
    }
    return null
}

function find_EtoRB_connector(to_id) {
    /* Find the connector that connects from from_id to to_id */

    var connlist = orm.roleboxes[to_id].datum().connectors;
    for (var n in connlist) {
        var cd = orm.connectors[connlist[n]].datum();
        if ( cd.conntype == conntypes.EtoRB ) { return connlist[n] }
    }
    return null
}
...
function assign_role_to_value(d_rolebox, d_conn) {

    /*
       If a connector connects a rolebox and value,
       1. Check that value will not play "primary" role
       2. Set the connection type
       3. The rolebox datum needs to be updated to include
          which value plays the role
       4. Set which overlays on the value the connection is eligible to
          link to.
     */

    // Check if value would be primary role (not allowed)
    var rbgroup = d3.select("#"+d_rolebox.parent);
    var boxID = get_primary_role(rbgroup);
    if ( d_conn.to == boxID ) {
        d_conn.to = "";
        return
    }
    // Default conntype
    d_conn.conntype = conntypes.VtoRB;
    // Set entity for rolebox
    d_rolebox.entity = d_conn.from;
    // Check flip condition for rolebox group and redraw connected connectors
    check_flip_and_connectors( rbgroup );
    // Set eligible overlay locations for linking connector
    d_conn.to_loc = eligible_rolebox_locations(d_conn.to, linkto="value");
}
...
function set_default_conntype(conn) {
    ...
    if ( d_to.kind == "rolebox" && d_from.kind == "value") {
...
function draw_connector(conn) {
    ...
    if ( cd.conntype == conntypes.default ) {
        ...
        conn
            ...
            .attr("class",`connector conn_mandatory${np}`) // conn_mandatory
