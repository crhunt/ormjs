...
function closest_on_line(pos,linepos) {
    ...
    var ipos = {x: ix, y:iy};
    // On line segment
    ipos = closest_to_segment(ipos,linepos);
...
function closest_to_segment(pos, linepos) {
    var minx = Math.min(linepos.x1, linepos.x2);
    var maxx = Math.max(linepos.x1, linepos.x2);
    pos.x = Math.max(minx,pos.x);
    pos.x = Math.min(maxx,pos.x);
    var miny = Math.min(linepos.y1, linepos.y2);
    var maxy = Math.max(linepos.y1, linepos.y2);
