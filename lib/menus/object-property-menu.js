...
ormjs.PropertyMenu = class {
    ...
    static draw_popup(object) {
        ...
        */
        ...
        var popfun = { "entity": function() { return ormjs.PropertyMenu.entity_form(object) },
                    "predicate": function() { return ormjs.PropertyMenu.fact_form(object) },
        ...
        if (object.datum().kind == "predicate") {
    ...
    static remove_all_popups(viewID,_callback) {
        
        /* Close all popup groups. This is done before saving svg. */
        
        // Get all popup groups
        var popup_groups = d3.select(`#${viewID}`).selectAll(".popup_group");
    ...
    static set_rbgroup_uconstraints(rbgroup) {
        ...
        var predicates = ormjs.models[rbgroup.datum().model].objects["predicate"];
    ...
    static update_name(object) {
        ...
        if (["entity","value","predicate"].includes(kind)) {
        ...
        */
        ...
        var rbg = d3.select("#popsvg-"+rbgroup.attr("id")).append("g")
            .datum( {x: x, y: y, dx: 0, dy: 0, x0: x, y0: y,
                    boxes: []} )
            .attr("class", "ormjs-predicate_prototype ormjs-poprbg")
            .attr("id", rbgID)
