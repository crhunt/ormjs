...
var popwidth = GraphUtils.get_css_number('--pop-width');
var popheight = GraphUtils.get_css_number('--pop-height');
...
function draw_popup(object) {
    ...
    var popup = svg.append("g")
        ...
        .attr("transform", Graph.translate(-popwidth/2, -popheight/2));
    popup.append("rect")
        .attr("class","popup")
        .attr("x", 0)
        .attr("y", 0)
...
function append_close(popup,object) {
    ...
    var xsize = GraphUtils.get_css_number('--pop-close-sz');
    var xmarg = 1.2*xsize;
...
function append_submit(popup, object) {
    ...
    // Dimensions
    var stroke_sz = GraphUtils.get_css_number('--pop-stroke');
    var stroke_sz_h = GraphUtils.get_css_number('--pop-stroke-hover');
    var chsize = { x: GraphUtils.get_css_number('--pop-ch-width'), 
                   y: GraphUtils.get_css_number('--pop-ch-height'),
                   m: GraphUtils.get_css_number('--pop-ch-marg') };
    var xmarg = GraphUtils.get_css_number('--pop-close-sz');
...
function popup_event(event,d) {
    ...
    var objID = Graph.get_parentID(click_objID);
    // Create popup
...
function pdragged(event,d) {
    ...
    d3.select(this)
        .attr("transform", Graph.translate(d.x, d.y));
}
...
function enter_last_popup() {
    ...
    open_popups = GraphUtils.remove_from_array(open_popups, popup);
}
...
function entity_form(entity) {
    ...
    if ( !orm.entities[entityID].check_independence() ) { ch += ` disabled="disabled"`; }
...
function value_form(value) {
    ...
    var ch = ""
    if (d.independent) { ch = `checked="checked"`}
    if ( !orm.values[valueID].check_independence() ) { ch += ` disabled="disabled"`; }
    return `
    <div class="popup_form">
    <h1>Value</h1>
        <table class="form_table">
...
function popup_rbgroup(rbgroup) {
    ...
    for (var n in boxes) {
        // Create box
        var d3rbox = Rolebox.draw_box({predicate: {d3object: rbg}})
        // Set initial uniqueness constraint to match rbgroup
        d3rbox.datum().multiplicity = d3.select("#"+boxes[n]).datum().multiplicity;
        d3rbox.datum().uclist = available_uconstraints(n, boxes.length);
        Rolebox.set_rolebox_iuc(d3rbox);
        d3rbox.on("click", rotate_uconstraint);
    }
...
function available_uconstraints(n,total) {
    var mult_param = Rolebox.multiplicity();
    var av = []
...
function allowed_uc_combination(rbg) {
    ...
    if ( ( uccnt[mult_param.many] > 0 && uccnt[mult_param.many] < total-1 ) ||
        ...
        Graph.class_as(rbgID,"notparsed");
        return
    }
    Graph.unclass_as(rbgID,"notparsed");
}
...
function popup_constraint(d3constraint) {
    ...
    // Dimensions
    var d = 2*GraphUtils.get_css_number('--constraint-radius');
    ...
    var supported = Constraint.supported_types().list;
    ...
    supported.map( (type) => {
        ...
        constraint_option.append("rect")
            ...
            .attr("width", w)
            .attr("height", w)
            .attr("transform", () => Graph.translate(-w/2,-w/2));
        // Draw the constraint itself
        Constraint.draw_constraint(constraint_option);

        d3.select(`#pop-cg-${id}-${type}`)
            .attr("transform", () => Graph.translate(pos.x+w/2+str,pos.y+w/2+str));
    });
    ...
    supported.map( (type) => {
        ...
        d3.select(`#pop-cg-${id}-${cptype}`)
            ...
            .on("mouseover", (event) => {
                ...
                targetID = Graph.levelupID(targetID);
                Graph.class_as(`r-${targetID}`,"hovered");
            ...
            .on("mouseout", (event) => {
                ...
                if (targetID == null) {return}
                targetID = Graph.levelupID(targetID);
                Graph.unclass_as(`r-${targetID}`,"hovered");
                document.getElementById(`type-${id}`).value = 
                    document.getElementById(`type-${id}`).lastvalue;
            } )
            ...
            .on("click", (event) => {
                ...
                if (targetID == null) {return}
                targetID = Graph.levelupID(targetID);
                supported.map( (t) => { Graph.unclass_as(`r-pop-cg-${id}-${t}`,"selected"); } );
                Graph.class_as(`r-${targetID}`,"selected");
                document.getElementById(`type-${id}`).value = 
                    d3.select(`#${targetID}`).datum().type;
                document.getElementById(`type-${id}`).lastvalue =
                    document.getElementById(`type-${id}`).value;
