...
function draw_popup(object) {
    ...
    var svg = ormjs.Graph.any_object(object.view).d3object;
...
function append_close(popup,object) {
    ...
    var xsize = ormjs.GraphUtils.get_css_number('--pop-close-sz');
    var xmarg = 1.2*xsize;
    var popwidth = ormjs.size.popup.width;
...
function enter_last_popup() {
    ...
    var popup = ormjs.propmenu.open_popups[open_popups.length-1];
...
function update_rolebox_fields(object) {
    ...
    if (boxes.length > 2) {
        var rng = ormjs.GraphUtils.range(boxes.length-2, startAt=2);
        for ( var n in rng ) {
...
function update_name(object) {
    ...
    if (["entity","value","rolebox_group"].includes(kind)) {
        objects[kind].update_display_name();
    }
...
function allowed_uc_combination(rbg) {
    ...
    if ( ( uccnt[mult_param.many] > 0 && uccnt[mult_param.many] < total-1 ) ||
        ...
        ormjs.Graph.class_as(rbgID,"notparsed");
        return
    }
    ormjs.Graph.unclass_as(rbgID,"notparsed");
}
...
function popup_constraint(d3constraint) {
    ...
    // Dimensions
    var d = 2*ormjs.GraphUtils.get_css_number('--constraint-radius');
    var str = ormjs.GraphUtils.get_css_number('--stroke-width');
