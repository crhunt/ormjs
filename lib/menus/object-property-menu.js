...
function constraint_form(d3constraint) {
    ...
    <h1>Constraint</h1>
        ...
        <td class="right_col"><input type="text" id="type-${constID}" 
            name="type" value="${d.type}" readonly /></td>
        </tr>
        <tr>
...
function popup_constraint(d3constraint) {
    ...
    d3.select("#popsvg-"+id).attr("width", (2*dx + w)*nwide);
    ...
    supported.map( (type) => {
        ...
        d3.select(`#pop-cg-${id}-${type}`)
            .attr("transform", () => translate(pos.x+w/2,pos.y+w/2));
    });
    ...
    supported.map( (type) => {
        ...
        d3.select(`#pop-cg-${id}-${cptype}`)
            ...
            .on("click", (event) => {
                ...
                document.getElementById(`type-${id}`).value = d3.select(`#${targetID}`).datum().type;
            } )
            .call(drag_form);
    });
