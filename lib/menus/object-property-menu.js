...
function constraint_form(d3constraint) {
    ...
    <h1>Constraint</h1>
        ...
        <td class="right_col"><input type="text" id="type-${constID}" 
            name="type" value="${d.type}" readonly disabled /></td>
        </tr>
        `;
    console.log("value row id", `value-row-${constID}`)
    form_html += `<tr id="value-row-${constID}">
    ...
    form_html += `
        <tr>
            ...
            <div height="${svgheight}" style="overflow-y:scroll">
...
function popup_constraint(d3constraint) {
    ...
    supported.map( (type) => {
        ...
        var content = "";
        if (type == "external-constraint") { content = ">2"; }
...
function contraint_value_viz(id,type) {
    ...
    if (type == "external-constraint" || type == "internal-constraint") {
