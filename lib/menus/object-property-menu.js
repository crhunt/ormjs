...
function constraint_form(d3constraint) {
    ...
    var svgid = "popsvg-"+d3constraint.attr("id");
...
function popup_constraint(d3constraint) {
    ...
    console.log(d3.select("#popsvg-"+id))
    var id = d3constraint.attr("id");
    if ( d3.select("#popsvg-"+id).empty() ) { return }

    console.log("not empty")

    var r = 2*parse_number( get_css_variable('--constraint-radius') );
    ...
    var m = -1;
    var p = 0;
    for (var name in constrainttypes) {
        var type = constrainttypes[name];
        p = n % nwide;
        if (p == 0) { m += 1};
        typeiter[type] = {x:p, y:m};
        n += 1;
    }

    console.log(typeiter)

    for (var type in typeiter) {
        console.log(type, typeiter[type])
        var pos = typeiter[type];
        var constraint_option = d3.select("#popsvg-"+id).append("g")
            .datum( { x: pos.x*dx, y: pos.y*dx, dx: pos.x*dx, dy: pos.y*dx, 
                      x0: 0, y0: 0,
        ...
        console.log(type, pos.x*dx, pos.y*dx)
        //constraint_option.attr("transform", () => translate(pos.x*dx,pos.y*dx));
    }
