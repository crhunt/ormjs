/* 
    ...
    a model update is triggered. So no incremental updates. However, the
...
ormjs.Metamodel = class {
    ...
    populate() {
        ...
        // Add entities
        this.add_objects(objects.entity,"EntityType");
        // Add values
        this.add_objects(objects.value,"ValueType");
        // Add all facts
        this.add_facts(objects.rolebox_group);
        // Add all constraints
        // TO DO
    ...
    display_xml() {
        ...
        var xml = (' ' + this.XML).slice(1); // Deep copy
        xml = xml.replaceAll('<','&lt;').replaceAll('>','&gt;');

        // Display
        d3.select(`#${model.xml_target}`).html(`<pre><code class="language-xml">${xml}</code></pre>`);
    }
    ...
    find_fact(fact) {
        ...
        */
        ...
        var shortFormat = ormjs.display.shortFormat;
        var graphFormat = ormjs.display.graphFormat;
