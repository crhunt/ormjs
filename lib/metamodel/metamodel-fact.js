...
class Fact {
    ...
    constructor(n, rbgroup) {
        ...
        this._rbgID = rbgroup.attr("id");
        this.InternalConstraints = [];
        this._Name = rbgroup.datum().name;
        this._Shadows = [ rbgroup.attr("id") ];
    ...
    populate_shadows() {
        ...
        if (matchlist.length > 0) {
            for (var n in matchlist) {
                var rbgIDn = key_from_value(metamodel.ormjs, matchlist[n]);
    ...
    fill_readings() {
        // Add entities to reading and rel reading for all readings in fact
        _fill_readings(this);
    }
