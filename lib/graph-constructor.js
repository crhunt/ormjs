...
function add_shadows() {
    ...
    /* Shadows for facts */
    // Unshadow all
    for (var objID in orm.rbgroups) { unclass_shadowed(objID);}
    for (var factID in metamodel.Fact) {
        ...
        if (shadows.length > 1) {
            ...
            for (var n in shadows) { class_shadowed(shadows[n]); }
        }
    }

    /* Shadows for entities and values */
    // Unshadow all
    for (var objID in orm.entities) { unclass_shadowed(objID);}
    for (var objID in orm.values) { unclass_shadowed(objID);}
...
function unclass_shadowed(anyID) {
    var class_string = d3.select("#"+anyID).attr("class");
    d3.select("#"+anyID).attr( "class", class_string.replaceAll(" shadowed", "") );
}

function class_shadowed(anyID) {
    var class_string = d3.select("#"+anyID).attr("class");
    d3.select("#"+anyID).attr("class", class_string + " shadowed");
}
