    ...
    static plays_lead_roles(object) { 
        ...
        roles.map( (roleID) => {
            ...
            if ( d3.select(`#${rbgID}`).datum().entity_in == object.id ) {
                    // It is! Add to list
                    rbgroups.push( orm.rbgroups[rbgID] );
            }
        });
...
function get_any_object(anyID) {
    ...
    if (orm.constraints.hasOwnProperty(anyID)) {
        return orm.constraints[anyID];
    }
...
function closest_overlay(position, anyID,locations=dragevent.locations) {
    ...
    if ( get_object_kind(anyID) == "constraint" ) {
        return orm.constraints[anyID].closest_overlay(position);
    }
    if ( get_object_kind(anyID) == "connector" ) {
