...
ormjs.Graph = class {
    ...
    static delete_object(object) {
        ...
        //ormjs.Graph.delete_reference(object);
        //delete object;
    ...
    static closest_overlay(position, anyID,locations=ormjs.default_dragevent) {
        ...
        if (anyID in objects[d.kind]) {
            return objects[d.kind][anyID].closest_overlay(position, locations)
        }

        /*if ( ormjs.Entity.is_entityID(anyID) ) {
    ...
    static closest_object(position,modelID) {
        ...
        */
        ...
        var subtypes = Connector.all_subtypes(modelID);
    ...
    static object_ref(anyID) {
        ...
        return refs[ ormjs.Graph.object_kind(anyID) ]
    }

    static any_object(anyID) {
        /* Get any main ORM object. */

        var objects = model_from_id(anyID).objects;
        var kind = object_kind(anyID);

        if (objects[kind].hasOwnProperty(anyID)) {
            return objects[kind][anyID];
        }
        return null
    }
