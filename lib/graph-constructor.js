...
class Graph {
    ...
    static dragged(event,d,object) {
        ...
        if (object.d3object.classed("selected")) {
            ...
            // Snap to entities
            d.dx = Graph.snap( d.dx + d.x0, "x", object.id ) - d.x0;
            d.dy = Graph.snap( d.dy + d.y0, "y", object.id ) - d.y0;
            d.x = d.x0 + d.dx;
            d.y = d.y0 + d.dy;
    ...
    static closest_overlay(position, anyID,locations=dragevent.locations) {
    ...
    static closest_location(position,xyo,locations=dragevent.all_locations) {
    ...
    static closest_object(position) {
        ...
        */
        ...
        objinfo = Graph.closest_of_kind(position, objinfo, subtypes);
    ...
    static closest_of_kind(position, objinfo, objdic) {
        ...
        for ( var anyID in objdic ) {
            ...
            distance = Graph.position_distance(position, candidateObject.datum());
    ...
    static snap( pos, field, anyID ) {
        ...
        // Closest entity
        var closest = Graph.closest_object_1D(anyID, orm.entities, field, pos);
        // Closest value
        var closest_v = Graph.closest_object_1D(anyID, orm.values, field, pos);
        if (closest_v.distance < closest.distance) { closest = closest_v }
        // Closest rolebox
        var closest_rb = Graph.closest_object_1D(anyID, orm.roleboxes, field, pos);
        ...
        var closest_f = Graph.closest_object_1D(anyID, orm.rbgroups, field+"c", pos);
        var field_suff = "";
    ...
    static draw_overlay(object,oclass) {
        ...
        overlay.append("circle")
            ...
            .attr("transform", () => Graph.translate(width/2, 0) );
        // Right circle overlay
        overlay.append("circle")
            .attr("class","overlay "+oclass)
            .attr("transform", () => Graph.translate(width, height/2) );
        // Bottom circle overlay
        overlay.append("circle")
            .attr("class","overlay "+oclass)
            .attr("transform", () => Graph.translate(width/2, height) );
        // Left circle overlay
        overlay.append("circle")
            .attr("class","overlay "+oclass)
            .attr("transform", () => Graph.translate(0, height/2) );
    ...
    static object_width(object) {
        ...
        d3.select(`#r-${object.id}`)
            .attr("width", width)
            .attr("transform", () => Graph.translate( -width/2, -height/2 ));
    ...
    static overlay_width(object) {
        ...
        var overlay = Graph.draw_overlay(object.d3object,ov);
        overlay.attr("transform", () => 
            Graph.overlay_translate(d.x0, d.y0, 
    ...
    static add_shadows() {
        ...
        /* Shadows for facts */
        // Unshadow all
        for (var objID in orm.rbgroups) { Graph.unclass_as(objID, "shadowed");}
        for (var factID in metamodel.Fact) {
            ...
            if (shadows.length > 1) {
                ...
                for (var n in shadows) { Graph.class_as(shadows[n], "shadowed"); }
            }
        }

        /* Shadows for entities and values */
        // Unshadow all
        for (var objID in orm.entities) { Graph.unclass_as(objID, "shadowed");}
        for (var objID in orm.values) { Graph.unclass_as(objID, "shadowed");}
...
class GraphUtils {
    ...
    static get_css_number(varname) {
        return GraphUtils.parse_number(GraphUtils.get_css_variable(varname))
    }
