...
function linePathArrow(end) {
    var start = 0;
    var offset = 6*GraphUtils.get_css_number('--stroke-width');
    end -= offset;
    return [
            "M", start, ",", 0,
            "L", end, ",", 0,
...
function constraintArrowPath(end) {
    var headRadius = GraphUtils.get_css_number('--stroke-width-wide');
    return arrowPath(headRadius,end)
};


function subtypePath(end) {
    var headRadius = 2*GraphUtils.get_css_number('--stroke-width');
    var offset = 1.5*GraphUtils.get_css_number('--stroke-width');
    end -= offset;
    return arrowPath(headRadius,end)
};
