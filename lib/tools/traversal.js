...
ormjs.Traversal = class {
    ...
    static select(objID) {
        ...
        */
        ...
        nodes.selected = d3.selectAll(`.ormjs-tselected`).nodes().map( (node) => {return node.id} );
    ...
    static set_traversal_classes(view) {
        ...
        */
        var in_view = view.objects_in_view();
        var selected = d3.selectAll(`.ormjs-tselected`).nodes().map( (node) => {return node.id} );
    ...
    static highlighted_predicates(view) {
        ...
        */
        var in_view = view.objects_in_view();
        console.log("in_view", in_view)
        var selected = d3.selectAll(`.ormjs-tselected`).nodes().map( (node) => {return node.id} );
        var overlap = selected.filter(value => in_view.includes(value));
        var predicates = overlap.map( (sID) => {
            if(sID.includes("predicate")) { return sID }
        }).filter(v=>v);
