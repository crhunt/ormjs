...
ormjs.Predicate = class {
    ...
    constructor(data) {
        ...
        if(!data.d3object) {
            ...
            this.id = ormjs.Predicate.generateID(this.model);
        ...
        // Update ORM
        if(!data.d3object) { ormjs.models[this.model].update(); }
    }
    ...
    static generateID(modelID) {
        ...
        return ormjs.Model.generateID(modelID,"predicate")
    }
    ...
    create_d3object(data) {
        ...
        // Datum
        var def_d = ormjs.Predicate.default_datum();
        ...
        var rbgroup = svg.append("g")
            .datum( d )
            .attr("id", this.id)
            .attr("class", "ormjs-predicate_prototype")
        ...
        rbgroup
            .append("text")
            .attr("class","ormjs-rbname")
            .attr("id","t-"+this.id)
            .attr("x", function(d){ return (d.x0) })
            .attr("y", function(d){ return (d.y0 - 1.5*ormjs.size.rolebox.height) })
        ...
        var osize = ormjs.size.rolebox.height;
        rbgroup
            .append("path")
            .attr("d", function(d) { return ormjs.Predicate.plusPath(d,osize) } )
            .attr("class","ormjs-rbadd")
            .attr("id", "add-"+this.id)
            .attr("transform", () => ormjs.Graph.translate(-ormjs.size.rolebox.width, 0) );
    ...
    add_rolebox() {
        ...
        */
        ...
        // Add box
        var rbox = new ormjs.Rolebox({predicate: this});
        ...
        if ( d.boxes.length > 1 ) {
            ...
            roleboxes[d.boxes[ind]].set_eligible_overlays();
    ...
    duplicate() {
        ...
        var offset = ormjs.size.rolebox.height*0.75;
        var x = gd.x + offset;
        var y = gd.y + offset;
        
        // Duplicate fact
        var predcopy = new ormjs.Predicate({x: x, y: y, view: this.view, model: this.model})
        ...
        var roleboxes = ormjs.models[this.model].objects.rolebox;
        roleboxes[ boxes[0] ].duplicate( roleboxes[ gdc.boxes[0] ] );
        var rng = ormjs.GraphUtils.range(boxes.length-1,1);
        for ( var n in rng ) {
            var rbox = predcopy.add_rolebox();
            roleboxes[ boxes[rng[n]] ].duplicate(rbox);
        }
    ...
    update_display_name() {
        ...
        // Add reverse name
        gd.rname = ormjs.Rolebox.format_name(gd.rname);
        if ( gd.rname.length > 0 ) { display_name += " / " + gd.rname; }
        // Set text field value and position
        d3.select(`#t-${this.id}`)
            .text(display_name)
            .attr("x", function() { 
                return (gd.x0 + (boxcount-1) * ormjs.size.rolebox.width/2) 
            });
    }
    ...
    namelist() {
        ...
        var rbname = boxes.map( (boxID) => {
            ...
            d.name = ormjs.Rolebox.format_name(d.name);
            // Push name to list
            return d.name;
        });
    ...
    move() {
        ...
        if (d.rotated) {
            this.d3object
                .attr("transform", () => ormjs.Graph.translate_rotate(d.x,d.y,d.dx,d.dy));
        } else {
            this.d3object
    ...
    actions() {
        ...
        d3.select("#"+`add-${this.id}`)
            ...
            .on("click", function(event) {
                var predID = ormjs.Graph.get_parentID( event.target.id.toString() );
                ormjs.Graph.any_object(predID).add_rolebox(); 
            });
    ...
    dragstarted(event,d) {
        // Initiate drag event
        var rbgroup = ormjs.Graph.dragstarted(event,d);
    ...
    position_boxes() {
        ...
        for (var n in gd.boxes) {
            ...
            if (gd.rotated) {
                d.y = gd.y + n * ormjs.size.rolebox.width;
                d.x = gd.x;
            } else {
                d.x = gd.x + n * ormjs.size.rolebox.width;
                d.y = gd.y;
            }
        }
    ...
    set_fact_center() {
        ...
        var d = this.d3object.datum();
        var shift = - 0.5 * ormjs.size.rolebox.width + (d.boxes.length) * ormjs.size.rolebox.width / 2;
        // Update central position of rbgroup
        if (d.rotated) {
            d.yc = d.y + shift;
            d.xc = d.x;
    ...
    flip() {
        ...
        // Redraw all connected connectors
        ormjs.Connector.redraw(gd.connectors);
    }
    ...
    align() {
        ...
        var y = gd.y0;
        for (var n in gd.boxes) {
            roleboxes[gd.boxes[n]].move(x,y);
            // Set eligible overlays for connectors
            roleboxes[gd.boxes[n]].set_eligible_overlays();
            x += ormjs.size.rolebox.width;
        }
    ...
    delete() {
        ...
        for (var n in boxes ) { delete roleboxes[ boxes[n] ]; }
    }
    ...
    role_players() {
        // All role players
        var boxes = this.d3object.datum().boxes;
        return boxes.map( (boxID) => {
            return d3.select(`#${boxID}`).datum().entity
        }).filter(v => v);
    }
...
ormjs.Rolebox = class {
    ...
    constructor(data) {
        ...
        if(!data.d3object) {
            this.model = data.predicate.model;
            this.view = data.predicate.view;
            // Create new d3 object
    ...
    create_d3object(data) {
        var rbox = ormjs.Rolebox.draw_box(data);
    ...
    static draw_box(data) {
        ...
        var boxID = ormjs.Rolebox.generate_boxID(boxcount,rbID)
        //var boxID = "r-" + boxcount.toString() + "-" + rbID;
        var rbname = "rolebox " + boxcount.toString() ;

        // Datum
        var def_d = ormjs.Rolebox.default_datum();
        var data_d = { x : d.x + boxcount * ormjs.size.rolebox.width, y : d.y,
                       dx : boxcount * ormjs.size.rolebox.width,
                       name : rbname,
                       parent: rbID };
        def_d = { ...data_d, ...def_d };
        var bd = ormjs.Graph.fill_datum(data, def_d);
        
        // Rolebox visualization
        rbgroup.append("g")
               .datum( bd )
               .attr("class","ormjs-rolebox_prototype")
               .attr("id", boxID)
               .attr("parent", rbID)
               .attr("x", function(){
                    return d.x0 + boxcount * ormjs.size.rolebox.width;
               })
               .attr("y", function(){
                    return d.y0;
               })
               .attr("width", ormjs.size.rolebox.width)
               .attr("height", ormjs.size.rolebox.height)
               .append("rect")
                    .attr("class","ormjs-rolebox")
                    .attr("id", "r-" + boxID)
                    .attr("width", ormjs.size.rolebox.width)
                    .attr("height", ormjs.size.rolebox.height)
                    .attr("x", function(){
                    ...
                    .attr("transform", () => ormjs.Graph.translate(-ormjs.size.rolebox.width/2 , 
                                                             -ormjs.size.rolebox.height/2));
        ...
        rbox
            ...
            .attr("class","ormjs-rb_constraint")
            .attr("id", `c-${boxID}`);
    
        return rbox
    }

    static generate_boxID(boxcount,predID) {
        var boxID = "r-" + boxcount.toString() + "-" + predID;
        return boxID
    }

    static predicate_from_boxID(boxID) {
        return ormjs.Graph.get_parentID( boxID )
    }

    add_overlay() {
        // Add rolebox overlay
        var overlay = ormjs.Graph.draw_overlay(this.d3object, "rboverlay");
    ...
    set_internal_uc() {
        ...
        ormjs.Rolebox.set_rolebox_iuc(this.d3object);
    }
    ...
    static set_rolebox_iuc(d3object) {
        ...
        uconstraints[ mult_param.many ] = ormjs.IUC.many;
        uconstraints[ mult_param.one ] = ormjs.IUC.unique;
        uconstraints[ mult_param.skip ] = ormjs.IUC.skip;

        d3.select("#c-"+d3object.attr("id"))
            .attr("d", () => uconstraints[d.multiplicity]({x: 0, y:0}) )
            .attr("transform", () => ormjs.Graph.translate(pos.x,pos.y));
    }

    static multiplicity() {
        return { none: "none", one: "one", many: "many", skip: "skip"}
    }
    // Data
    ...
    label(rbname) {
        let d = this.d3object.datum();
        if(arguments.length > 0) { 
            d.name = ormjs.Rolebox.format_name(rbname);
            let pred = ormjs.models[d.model].objects["predicate"][d.parent];
            pred.update_display_name();

        } else {
            return this.d3object.datum().name
        }
    }
    ...
    actions() {
        ...
        var rboxMenu = ormjs.OptionMenu.rolebox_menu(this.model);
        this.d3object
            .on("contextmenu", d3.contextMenu(rboxMenu)) // Right click menu
    ...
    move(x,y) {
        ...
        d3.select("#c-"+this.id)
            .attr("transform", () => ormjs.Graph.translate(x,y) );
        // Move the overlays
    ...
    mousedown(event, rbox) {
        ...
        */
        ...
        var mousepos = {x: m[0] + d.x - ormjs.size.rolebox.width/2, 
                        y: m[1] + d.y - ormjs.size.rolebox.height/2};
        if (gd.rotated) {
            mousepos = {y: m[0] - ormjs.size.rolebox.width/2 + d.y,
                        x: -m[1] + ormjs.size.rolebox.height/2 + d.x}
        }

        ormjs.Graph.mousedown(event, rbox, mousepos);
    }
    ...
    flip_mandatory() {
        ...
        for (var n in d.connectors ) {
            if ( d3.select("#"+d.connectors[n]).datum().conntype == conntypes.EtoRB ||
                ...
                var conn = objects.connector[d.connectors[n]];
            }
        }
        // Redraw all connected connectors
        ...
        if (d.mandatory) {
            ...
            //update_name(obj);
            ormjs.Graph.any_object(d.entity).update_display_name();
        }
    ...
    remove(event,d) {
        ...
        var rboxID = ormjs.Graph.levelupID( event.target.id.toString() );
        var rbox = ormjs.Graph.any_object(rboxID);
        if (rbox == null) { return }
    ...
    delete() {
        ...
        var objects = ormjs.models[this.model].objects;
        var rbgroup = objects["predicate"][ this.parent ];
        ...
        conns.map( (connID) => {
            ...
            gd.connectors = ormjs.GraphUtils.remove_from_array( gd.connectors, connID );
            // Delete the connector
            objects.connector[connID].delete();
        });

        // Remove box reference from parent
        gd.boxes = ormjs.GraphUtils.remove_from_array( gd.boxes, this.id );

        // Remove box reference from records
        delete objects.rolebox[ this.id ];

        // Remove the rolebox visualization
        this.d3object.remove();
        if (gd.flipped) { 
            // Adjust center of rolebox group to new left rolebox
            gd.dx += ormjs.size.rolebox.width;
            gd.x = gd.x0 + gd.dx;
            rbgroup.move();
            rbgroup.align();
        ...
        objects.rolebox[ gd.boxes[gd.boxes.length-1] ].set_eligible_overlays();
    ...
    closest_overlay(pos,locations=ormjs.default_dragevent) {
        ...
        return ormjs.Graph.closest_location(pos,xyo,locations=locations)
    }
    ...
    overlay_positions() {
        ...
        var xyoverlay = {};
        gd.rotated ? xyoverlay = ormjs.Graph.rotated_overlay_positions(this)
                   : xyoverlay = ormjs.Graph.overlay_positions(this);
        return xyoverlay
    }
    ...
    set_eligible_overlays() {
        ...
        conns.map( (connID) => {
            connectors[connID].set_eligible_overlays();
        });
    ...
    bubble_connectors() {
        ...
        var predicates = ormjs.models[this.model].objects["predicate"];
        // Set connectors at the group level
        predicates[this.parent].bubble_connectors();
        predicates[this.parent].set_subject();
    }
    ...
    eligible_locations(linkto="entity", loc=ormjs.default_dragevent) {
        ...
        */
        ...
        var rblocations = { "left": left_rb, "right": right_rb, 
                            "middle": middle_rb, "only": ormjs.default_dragevent };
    ...
    entity_connector() {
        ...
        for (var n in connlist) {
            var cd = connectors[connlist[n]].d3object.datum();
