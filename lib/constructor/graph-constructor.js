...
ormjs.Graph = class {
    ...
    static dragged(event,d) {
        ...
        if (object.d3object.classed("ormjs-selected")) {
            ...
            if (object.kind == "predicate") { object.position_boxes(); }
        }
    }
    ...
    static delete_object(object) {
        ...
        // ** Remove the object from records **
    ...
    static snap( pos, field, anyID ) {
        ...
        var obj_options = ["entity", "value", "rolebox", "predicate"];
        var field_suff = "";
        obj_options.map( (obj) => {
            var f = field;
            if (obj == "predicate") { f += "c"; }
            var _closest = ormjs.Graph.closest_object_1D(anyID, objects[obj], f, pos);
            if( _closest.distance < closest.distance ) {
                closest = _closest;
                if (obj == "predicate") { field_suff = "c"; }
            }
        });
    ...
    static object_kind(anyID) {
        ...
        if ( anyID.includes("predicate") &&
             anyID.includes("r-") &&
             ! anyID.includes("r-r-") ) { return "rolebox" }
        if ( anyID.includes("predicate") ) { return "predicate" }
        return ""
    }

    static change_id(d3object, new_id) {
        var child_nodes = d3object.node().childNodes;
        var root_id = d3object.attr("id");
        for (var n in child_nodes) {
            if ( child_nodes[n].id ) {
                var replace_id = child_nodes[n].id.replaceAll(root_id, new_id);
                d3.select(`#${child_nodes[n].id}`).attr("id", replace_id);
            }
        }
        d3object.attr("id", new_id);
        d3object.datum().id = new_id;
    }

    static object_number(anyID) {
    ...
    static draw_overlay(object,oclass) {
        ...
        var overlay = object.append("g")
            .attr("id", overlayID)
            .attr("class","ormjs-overlay_prototype")
            .attr("width", width)
            .attr("height", height)
            .attr("x", 0)
            .attr("y", 0)
        ...
        overlay.append("circle")
            .attr("class","ormjs-overlay "+oclass)
    ...
    static add_shadows(metamodel) {
        ...
        /* Shadows for facts */
        // Unshadow all
        for (var objID in objects["predicate"]) { 
            ormjs.Graph.unclass_as(objID, "ormjs-shadowed");
        }
        for (var factID in metamodel.Fact) {
            ...
            if (shadows.length > 1) {
                ...
                for (var n in shadows) { 
                    ormjs.Graph.class_as(shadows[n], "ormjs-shadowed"); 
                }
            }
        }
