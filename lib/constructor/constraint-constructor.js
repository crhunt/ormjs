...
ormjs.Constraint = class {
    ...
    param = {
        ...
        stroke : ormjs.size.constraint.stroke
    };
    ...
    create_d3object(data) {
        ...
        var constraint = svg.append("g")
            ...
            .attr("class", "ormjs-constraint_prototype")
            .attr("id", this.id)
            .attr("parent", this.id)
        ...
        constraint.append("circle")
            .attr("class", `ormjs-overlay coverlay`)
            .attr("id", `o-${this.id}`)
    ...
    type(typeval,content="") {
        let d = this.d3object.datum();
        if(arguments.length > 0) { 
            let allowed = ormjs.Constraint.supported_types();
            if (typeval in allowed.list) {
                d.type = typeval;
                this.set_content(content);
                this.redraw();
            }
        } else {
            return { "type": d.type, "content": d.content }
        }
    }
    ...
    set_radius() {
        ...
        // Default radius
        var r = ormjs.size.constraint.radius;
        var or = ormjs.size.constraint.oradius;
    ...
    static draw_constraint_path(d3parent) {
        ...
        if (path != null) {
            ...
            d3parent.append("path")
                .attr("d", path)
                .attr("class", "ormjs-constraint_val " + constrainttype)
                .attr("id", `val-${id}`);
            return
        }
        ...
        d3parent.append("text")
            ...
            .attr("class", "ormjs-constraint_text " + constrainttype)
            .attr("id", `val-${id}`)
    ...
    static constraint_path(d3object, r=-1) {
        ...
        var paths = {
            "inclusive-or": () => {return ormjs.ConstraintPath.inclusive_or(r) },
            "exclusion": () => {return ormjs.ConstraintPath.exclusion(r) },
            "exclusive-or": () => {return ormjs.ConstraintPath.exclusive_or(r) },
            "equality": () => {return ormjs.ConstraintPath.equality(r) },
            "identifier": () => {return ormjs.ConstraintPath.identifier(r) },
            "preferred-identifier": () => {return ormjs.ConstraintPath.preferred_identifier(r) }
        };
    ...
    duplicate() {
        ...
        var data = { x : d.x + offset, y : d.y + offset, view: this.view,
                     model: this.model };
    ...
    actions() {
        ...
        this.d3object
            ...
            .on("click", ormjs.Graph.remove) // Ctrl+click --> remove constraint
    ...
    closest_overlay(pos) {
        ...
        return ormjs.Geometry.point_on_circle(posdata)
    }
