...
function download_png(event) {
    ...
    var setw = Math.round( scrnw*4 * parse_number(get_css_variable("--canvas-w"))/100 );
    var seth = Math.round( scrnh*4 * parse_number(get_css_variable("--canvas-h"))/100 );
    var svgw = svg.attr("width");
    var svgh = svg.attr("height");
    svg
        .attr("width", setw)
        .attr("height", seth);
    console.log(scrnw, scrnh, setw, seth)
    ...
    //var svgBlob = new Blob([rawSvg], {type: 'data:image/svg+xml;base64'});
    //var svgBlob = "data:image/svg+xml;base64," + utoa( rawSvg );
    ...
    img.onload = function () {
        ...
        if (trigger) {
            console.log("download_png_inner triggered")
            var imgURI = canvas
                .toDataURL('image/png')
...
function populate_orm_state(class_name, orm_object_name) {
    ...
    var orig_format = ["entity_prototype", "value_prototype", "rolebox_group", 
                       "rolebox_prototype"];
    var new_format = ["constraint_prototype", "connector"];
    ...
    if (new_format.includes(class_name)) {
        ...
        for ( var k in object_divs.nodes() ) { 
            ...
            console.log(d3object.datum())
            // Create object
            if( class_name == "connector") { 
                var obj = new Connector({d3object: d3object}); 
                obj.record();
            }
            if( class_name == "constraint_prototype") { new Constraint({d3object: d3object}); }
        }
    }
