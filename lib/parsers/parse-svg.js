...
function download_png(event) {
    ...
    d3.select("#downloadPngButton").attr("download", () => {
        return download_name( d3.select("#downloadPngButton").attr("download") );
    });
...
function download_svg() {
    ...
    d3.select("#downloadSvgButton").attr("download", () => {
        return download_name( d3.select("#downloadSvgButton").attr("download") );
    });
...
function download_name(df) {
    if(!(d3.select("#uploadname").html()  === "")) {
        return d3.select("#uploadname").html();
    }
    return df
}
