...
function parse_orm() {
    ...
    var model = ormjs.models['id-model-0'];

    initialize_metamodel(model);
    ...
    Graph.add_shadows(metamodel);

}

function initialize_metamodel(model) {
    metamodel = new Metamodel("metamodel-0",model); 
}
