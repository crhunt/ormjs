...
ormjs.SVG = class {
    ...
    static upload(file, target_view) {
        /*
        ...
        */
        ...
        reader.onload = function(event) { 
            ...
            var view = ormjs.Graph.any_object(target_view);
        
            /* Add new svg to parent */
            ormjs.SVG.to_view(view, data);
    ...
    static create_model_ids_from_view(view) {
        ...
        var class_type = ["connector_prototype", "rolebox_group", 
                          "constraint_prototype", 
                          "entity_prototype", "value_prototype"];
        var class_kind = { 
            "connector_prototype": "connector", 
            "rolebox_group": "rolebox_group", 
    ...
    static populate_model_from_view(view) {
        /*
            ...
            1. Populate the datum of each object with the JSON string that was
               set at download.
            2. Update all datum references to the new id names used to add the svg to
               the model.
        ...
        */
        ...
        console.log("All objects", ormjs.models[view.model].objects)
    }
    ...
    static update_all_references(anyID, idref) {
        
        // Get datum
        var d = d3.select(`#${anyID}`).datum();
        var conn_targets = ["entity_prototype", "value_prototype", "rolebox_prototype",
                            "constraint_prototype", "connector_prototype"];
        var obj_targets = ["entity_prototype", "value_prototype"];
        
        // Update connector references
        if (d.connectors) {
            ormjs.SVG.update_reference_list(d.connectors, idref["connector_prototype"]);
        }
        if (d.kind == "connector") {
            conn_targets.map( (tp) => {
                d.from = ormjs.SVG.update_reference_id(d.from, idref[tp]);
                d.to = ormjs.SVG.update_reference_id(d.to, idref[tp]);
            } );
        }
        if (d.kind = "rolebox_group") {
            ormjs.SVG.update_reference_list(d.boxes, idref["rolebox_prototype"]);
            obj_targets.map( (tp) => {
                d.entity_in = ormjs.SVG.update_reference_id(d.entity_in, idref[tp]);
            } );
        }
        if (d.kind = "rolebox_prototype") {
            obj_targets.map( (tp) => {
                d.entity = ormjs.SVG.update_reference_id(d.entity, idref[tp]);
            } );
        }
    }

    static update_reference_id(rid, refmap) {
        if (rid in refmap) { return refmap[rid] }
        else { return rid }
    }

    static update_reference_list(reflist, refmap) {
        reflist = reflist.map( (rid) => {
            return ormjs.SVG.update_reference_id(rid, refmap)
        });
    }
