...
ormjs.GenerateRel = class {
    ...
    static parse(metamodel) {
        ...
        // Add highlight
        ormjs.RelHighlighter.highlight(metamodel.model);
    ...
    static display_rel(metamodel) {
        ...
        var fact_groups = ormjs.GenerateRel.facts_by_entity(metamodel);
    ...
    static highlight(modelID) {
        ...
        hlobjects.map( (obj) => {
            for ( var objID  in objects[obj] ) {
                ormjs.RelHighlighter.class_as_notparsed(objID,metamodel);
            }
        });
    
        // Find internal UCs that are not parsed
        for ( var boxID in objects.rolebox ) {
            ormjs.RelHighlighter.class_as_notparsed("c-"+boxID,metamodel);
        }
    }
    
    static class_as_notparsed(anyID,metamodel) {
        ormjs.Graph.unclass_as(anyID, "notparsed");
        if ( ! metamodel.relIDs.includes(anyID) && ormjs.display.highlightNoParse ) {
