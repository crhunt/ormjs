...
ormjs.GenerateRel = class {
    ...
    static display_rel(model) {
        ...
        // Display
        if (model.rel_target != null) {
            if (d3.select(`#${model.rel_target}`) != null) {
                d3.select(`#${model.rel_target}`)
                  .html(`<pre><code class="language-js">${relcode}</code></pre>`);
            }
        }
