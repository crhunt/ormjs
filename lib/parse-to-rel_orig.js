...
function parse_rolebox_fact(k) {
    ...
    facts[k].connector = name_to_relname( namelist.join(" ") );
...
function rolebox_entities(boxes) {
    ...
    for (var n in boxes) {
        ...
        var e_connectors = get_any_object( entityID ).datum().connectors;
...
function nary_fact_to_rel(fact) {
    ...
    for (var n in fact.keylist) {
        fact.keylist[n].datum().refmode ? atom = fact.keylist[n].datum().refmode : atom = fact.keylist[n].datum().name.substring(0,1);
        var keyname = { atom: name_to_relname(atom), //"id"+n.toString(),
    ...
    var ic_statement = `ic { ${fact.connector} ⊆ (`
    for (var n in keylist) {
        ic_statement += `${keylist[n].relname}, `
    }
    ic_statement += `${relobject}) }\n`;
    statements.push(ic_statement);
}
