...
function duplicate_entity(entity) {
    ...
    // Duplicate entity
    var ecopy = draw_entity(x,y);
    var dc = ecopy.datum()

    // Set entity data to match
...
function entity_actions(entity) {
    ...
    var eoverlay = d3.select( "#" + overlay_from_ID(entity.attr("id")) );
...
function format_refmode_name(initname, type="popular") {
    ...
    if (type == "popular") {
        ...
        var newname = initname.split(" ").join("_");
        return newname
    }
    return initname
}
...
function check_independence(obj) {
    var roles = object_plays_roles(obj);
    for (var n in roles) {
...
function edragged(event,d,entityID) {
    ...
    if (d3.select("#"+entityID).classed("selected")) {
        drag_selected(event);
    } else {
...
function entity_plays_lead_roles(entity) {
    ...
    var roles = object_plays_roles(entity);

    roles.map( (roleID) => {
        rbgID = d3.select("#"+roleID).datum().parent;
...
function object_plays_roles(object) {
    
    /* Find all roles played by object */

    var conns = object.datum().connectors;
    var roles = [];
    // Iterate through all connections of the entity
    conns.map( (connID) => {
        // Datum of connector
        console.log("object_plays_roles", "#"+connID)
...
function eomousedown(event) {
    ...
    // mouse position
    var pos = { x1: eo.x, x2: mousepos.x, y1: eo.y, y2: mousepos.y };
