...
ormjs.Entity = class {
    ...
    static generateID(model) {
        ...
        return ormjs.Model.generateID(model,"entity")
    }
    ...
    create_d3object(data) {
        ...
        ormjs.Graph.draw_overlay(entity,"eoverlay");
    ...
    update_display_name() {
        ...
        var ref_name = ormjs.Entity.format_refmode(d.refmode, d.reftype);
    ...
    record() {
        ...
        ormjs.models[this.model].objects.entity[this.id] = this;
    }
    ...
    actions() {
        ...
        var drag_entity = d3.drag()
            .on("start", (event,d) => {this.dragstarted(event,d,this)} )
            .on("drag", (event,d) => {this.dragged(event,d,this)} )
            .on("end", (event,d) => {this.dragended(event,d,this)});

        // Add events to the entity
        var entityMenu = OptionMenu.entity_menu(this.model);
        this.d3object
            .on("dblclick", popup_event)
            .on("contextmenu", d3.contextMenu(entityMenu)) // Right click menu
            .on("click", (event) => {this.remove(event,this.model)}) // Ctrl+click --> remove entity
            .call(drag_entity);
    ...
    dragstarted(event,d,entity) {
        ormjs.Graph.dragstarted(event,d,entity);
    }

    dragged(event,d,entity) {
        ormjs.Graph.dragged(event,d,entity);
    }

    dragended(event,d,entity) {

        /* End drag event for entity */

        ormjs.Graph.dragended(entity);
    ...
    static remove(event,modelID) {
        ...
        if (event.ctrlKey || event.buttons == 2) {
            ...
            ormjs.models[modelID].objects.entity[parentID].delete();
        }
    }
