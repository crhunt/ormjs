...
function entity_definition(x,y,entityID) {
    ...
    var entity = svg.append("g")
        .datum( {x: x, y: y, dx: 0, dy: 0, x0: x, y0: y,
                ...
                connectors: [],
                name: ename, refmode: "id", reftype: "popular"} )
...
function eoverlay_positions(entityID) {
    ...
    var width = entity.attr("width");
    var height;
    //entity.datum().kind == "entity" ? height = entity_param.height : height = value_param.height;
    var height = parseInt( entity.attr("height") );
    console.log(entityID, " width ",width," height ", height)
    var xyoverlay = {
        "bottom" : { x: x,
                  y : y + height/2, 
                  location : "bottom" },
