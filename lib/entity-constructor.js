...
function draw_entity(x,y) {
    ...
    function remove_entity(event,d) {
        if (event.ctrlKey) {
            ...
            entityID = d3.select(this).attr("id");

            // Remove relationships
            rels = d3.select(this).datum().relationships;
            for ( n in rels ) {
                console.log(rels[n]);
    ...
    /* With prototype. Want to use this, but need custom text. */
...
function svg_mouseup(event, rel) {
    ...
    if ( to_what.object.datum().kind == "entity" ) {
        if ( to_what.distance < entity_param.width ) {
            ...
            to_pos = closest_eoverlay(entities[dr.from].datum(), 
