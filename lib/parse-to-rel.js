...
function parse_to_rel() {
    ...
    fill_readings();
    console.log("relIDs", metamodel.relIDs);
...
function add_rel_readings() {
    ...
    for (var factID in metamodel.Fact) {
        ...
        for (var rnum in metamodel.Fact[factID].ReadingOrder[ronum].Reading) {
            var reading = metamodel.Fact[factID].ReadingOrder[ronum].Reading[rnum];
            // Forward Reading
            if ( reading._ReadingType == "NaryForwardReading" ) { add_NaryForwardReading(reading); }
            // Mandatory Reading
            if ( reading._ReadingType == "MandatoryForwardReading" ) { add_MandatoryForwardReading(reading); }
            // Internal UC Reading
...
function add_MandatoryForwardReading(reading) {
    ...
    if (total_roles == 2) {
        // Binary
        rolen == 0 ? relstring = `ic { total({${rolen}}, ${relname}) }` 
                   : relstring = `ic { total({${rolen}}, transpose[${relname}]) }`;
    } else {
        // Nary
        var atom = `{a${rolen}}`; // Get atom of role player
        var position = "_,".repeat(rolen) + atom + ",_".repeat(total_roles - rolen - 1);
        var relstring = `ic { {${rolen}}(${atom}) implies ${relname}(${position}) }`;
    }
...
function fill_reading(fact) {
    ...
    for (var n in fact.FactRoles) {
        ...
        if (objID != null) { 
            ...
            metamodel.relIDs.push.apply(metamodel.relIDs, metamodel.Object[objID]._Shadows);
        }
    }
    ...
    for (var r in fact.ReadingOrder[ro].Reading) {
        ...
        if ( data.includes("{") ) {
            ...
            metamodel.relIDs.push.apply(metamodel.relIDs, fact.ReadingOrder[ro].Reading[r]._includedIDs);
        }
