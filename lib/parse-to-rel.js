...
/* Determine shadowed objects from relnames */
/*
function shadowed_reading_orders() {

    var namelist = [];
    for ( var factID in metamodel.Fact ) {
        for ( var n in metamodel.Fact[factID].ReadingOrders ) {
            if ( metamodel.Fact[factID].ReadingOrders[n].RelName ) {
                namelist.push( metamodel.Fact[factID].ReadingOrders[n].RelName );
            }
        }
    }
    var duplicates = duplicate_names(namelist);
    for ( var factID in metamodel.Fact ) {
        for ( var n in metamodel.Fact[factID].ReadingOrders ) {
            if ( duplicates.includes( metamodel.Fact[factID].ReadingOrders[n].RelName ) ) {
                metamodel.Fact[factID].ReadingOrders[n]["Shadowed"] = true;
            } else {
                metamodel.Fact[factID].ReadingOrders[n]["Shadowed"] = false;
            }
        }
    }
...
function duplicate_names(namelist) {
...
function shadow_facts_from_metamodel() {
    for ( var rbgID in orm.rbgroups ) {
        unclass_shadowed(rbgID);
        if ( metamodel.Fact[rbgID].ReadingOrders[0].Shadowed ) { class_shadowed(rbgID); }
    }
}
*/
