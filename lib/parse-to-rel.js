...
function reading_order_relname(factID,roID) {
    ...
    if (rplayer) { relname = `${rplayer}`; }
    else { return false }
    if (!shortFormat) { relname += `${c}${metamodel.Fact[factID].FactRoles[boxes[0]].RelName}`; }
    
    // Add second role
    if (boxes.length > 1) {
        if ( metamodel.Fact[factID].FactRoles[boxes[1]].RelName.length > 0 ) {
            if (!shortFormat) { relname += `_${metamodel.Fact[factID].FactRoles[boxes[1]].RelName}`; }
        }
        rplayer = entity_relname_from_role(boxes[1],factID);
        if (rplayer) { relname += `${c}${rplayer}`; }
        else { return false }
        
        // Add additional roles
        var rng = range(boxes.length-2, startAt=2);
        for ( var n in rng ) {
            if (!shortFormat) { relname += `${c}${metamodel.Fact[factID].FactRoles[boxes[rng[n]]].RelName}`; }
