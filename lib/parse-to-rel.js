...
function parse_rolebox_fact(k) {
    ...
    facts[k].relation = name_to_relname( namelist.join("_").replaceAll(" ","") );
...
function rolebox_entities(boxes) {
    ...
    for (var n in boxes) {
        ...
        // Get relation between entity and rolebox to add to idlist
        var rb_relationships = orm.roleboxes[ boxes[n] ].datum().relationships;
        var e_relationships = orm.entities[ entityID ].datum().relationships;
        var overlap = rb_relationships.filter(value => e_relationships.includes(value));
