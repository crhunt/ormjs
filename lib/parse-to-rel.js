...
function reading_order_relname(factID,roID) {
    ...
    */
    ...
    if (boxes.length == 1) {
        ...
        var role = metamodel.Fact[factID].FactRoles[boxes[0]].RelName;
        if (rplayer) {
...
function add_rel_readings() {
    ...
    for (var factID in metamodel.Fact) {
        for (var ronum in metamodel.Fact[factID].ReadingOrders) {
        for (var rnum in metamodel.Fact[factID].ReadingOrders[ronum].Readings) {
            metamodel.Fact[factID].ReadingOrders[ronum].Readings[rnum].RelData = []; // Default
            // Forward Reading
            if ( metamodel.Fact[factID].ReadingOrders[ronum].Readings[rnum]._ReadingType == "NaryForwardReading" ) {
                metamodel.Fact[factID].ReadingOrders[ronum].Readings[rnum].RelData =
...
function add_NaryForwardReading(factID, ronum) {
    ...
    var relname = metamodel.Fact[factID].ReadingOrders[ronum].RelName;
    if (!relname) { return [] }

    // Create subset relation with all members of each entity, 
    // ordered by the arity of relname.
    var roleseq = metamodel.Fact[factID].ReadingOrders[ronum].RoleSequence;
...
function fill_reading(fact) {
    ...
    for (var n in fact.FactRoles) {
        ...
        if (objID != null) { 
            ...
            role_map["rel"][key] = metamodel.Object[objID].RelName; 
        }
    }

    for (var ro in fact.ReadingOrders) {
    for (var r in fact.ReadingOrders[ro].Readings) {
        var data = fact.ReadingOrders[ro].Readings[r].Data;
        var reldata = [...fact.ReadingOrders[ro].Readings[r].RelData];
        ...
        fact.ReadingOrders[ro].Readings[r].Statement = data;
        fact.ReadingOrders[ro].Readings[r].RelStatement = reldata;
    }
    }
}
...
function display_rel() {
    ...
    for ( var objID in fact_groups ) {
        ...
        for (var fn in fact_groups[objID]["facts"]) {
            ...
            var readings = metamodel.Fact[loc[0]].ReadingOrders[loc[1]].Readings;
            for (var r in readings) {
                if (readings[r].Statement != null) {
                    statements.push( `// ${readings[r].Statement}` );
                    statements.push.apply( statements, readings[r].RelStatement );
                }
            }
        }
    ...
    Prism.highlightAll(); // Don't want this here, but tricky to listen for innerhtml change
}
...
function facts_by_entity() {
    ...
    for ( var factID in metamodel.Fact ) {
        for ( var roID in metamodel.Fact[factID].ReadingOrders ) {
            var objID = metamodel.Fact[factID].ReadingOrders[roID]._FirstEntity;
            if (objID != null ) {
